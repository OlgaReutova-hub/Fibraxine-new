...
def build_patient_memo_html(data: dict, kcal: int) -> str:
    ...
    # Во встроенном просмотре (Streamlit `components.html`) относительные пути к
    # картинкам не работают — инлайним ассеты в HTML как base64.
...
def render_patient_memo_panel(memo_html: str) -> None:
    """Памятка вне колонок: на облаке `st.dialog` часто превращается в узкий сайдбар."""
    st.divider()
    st.subheader("Памятка для пациента")
    close_col, _ = st.columns([0.22, 0.78])
    with close_col:
        if st.button("Закрыть", key="patient_memo_close"):
            st.session_state.patient_memo_open = False
            st.session_state.patient_memo_html = None
            st.rerun()
...
if "patient_memo_open" not in st.session_state:
    st.session_state.patient_memo_open = False
if "patient_memo_html" not in st.session_state:
    st.session_state.patient_memo_html = None
with st.container():
...
if show_button:
    ...
    # Новый рацион — старая памятка неактуальна: закрываем, чтобы не путать с новым меню.
    st.session_state.patient_memo_open = False
    st.session_state.patient_memo_html = None
    ...
    if scenario is None:
        ...
        with right_col:
            ...
            if memo_clicked:
                ...
                st.session_state.patient_memo_html = build_patient_memo_html(data, kcal)
                st.session_state.patient_memo_open = True

        if st.session_state.patient_memo_open and st.session_state.patient_memo_html:
            render_patient_memo_panel(st.session_state.patient_memo_html)
